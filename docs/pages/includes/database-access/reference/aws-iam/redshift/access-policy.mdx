...
| Statement | Purpose |
| --------- | ------- |
|`RedshiftConnectAsDBUser` (required) | Connect to a database as an existing database user. |
|`RedshiftConnectAsIAMRole` (required) | Assume an IAM role to connect to a database with permissions mapped into the database 1:1 from the role's IAM permissions. |
|`RedshiftFetchMetadata` (optional) | Automatically import AWS tags as database labels or find missing information such as the database's AWS region. |
