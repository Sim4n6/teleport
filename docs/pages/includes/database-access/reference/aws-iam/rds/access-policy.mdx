...
| Statement | Purpose |
| --------- | ------- |
|`RDSAutoEnableIAMAuth` (optional) | Automatically enable IAM auth on RDS instances and Aurora clusters. |
|`RDSConnect` (required) | Generate an IAM authentication token to connect to a database. |
|`RDSFetchMetadata` (optional) | Automatically import AWS tags as database labels or find missing information such as the database's AWS region. |
