...
exception is rotating the `db` and `db_client` CAs. 

|CA type|Certificate subjects|
|---|---|
|[`host`](#host)|Teleport Agents. Auth Service and Proxy Service instances.|
|[`user`](#user)|Teleport users. Teleport SSH Service and Windows Desktop Service instances.|
...
2](#step-2-4-start-a-manual-rotation).

If you are joining Teleport processes to a cluster via the Teleport Auth Service
using a [join token](../../agents/join-services-to-your-cluster/join-token.mdx),
each Teleport process will need a CA pin to trust the Auth Service. The CA pin
will change after each `host` CA rotation. Make sure you use the *new* CA pin
when adding Teleport services after `host` CA rotation.

### `user`

The `user` CA issues a certificate when a user authenticates to Teleport. It
also signs client certificates for the Windows Desktop Service and Teleport SSH
Service. Teleport-protected servers and Windows desktops use these certificates
...
guide](../../desktop-access/active-directory-manual.mdx) to exporting the
Teleport user CA so the Windows Desktop Service can authenticate to RDP hosts.
...
Consult the appropriate [documentation](../../database-access/guides) for
configuring your databases before proceeding to the `update_clients` rotation
phase.
...
Teleport](../../server-access/openssh.mdx). Clients verify these certificates
when connecting to Teleport-protected OpenSSH servers.

If you used the [manual
method](../../server-access/openssh/openssh-manual-install.mdx) to enroll any
...
Elasticsearch](../../application-access/jwt/elasticsearch.mdx).
...
The `oidc_idp` CA signs messages sent by the Teleport OIDC IdP integration. AWS
verifies these messages to authenticate your Teleport account for features like
EICE EC2 Access, External Audit Storage, Auto-Discovery, and AWS Sync for Access
Graph. There are two ways to serve the public key for the Teleport `oidc_idp`
certificate authority:

- From the `/.well-known/jwks-oidc` path of the Teleport Proxy Service Web API.
- From an Amazon S3 bucket.

The `/.well-known/jwks-oidc` path of the Teleport Proxy Service Web API is
always enabled. To check whether you have enabled an S3 bucket for the Teleport
OIDC public key, run the following command:

```code
$ tctl get integrations --format=json | jq '.[] | select(.sub_kind == "aws-oidc")'
```

If you have enabled the S3 bucket, the `spec.aws_oidc.issuer_s3_uri` and
`spec.aws_oidc.role_arn` fields will be nonempty.

If you use the Web API endpoint, then you do not need to take further action.
The Teleport Proxy Service updates the endpoint automatically.

If you choose to use an S3 bucket, then you need to upload both the original and
new public keys to S3:

1. After the `init` phase, run the following script to upload both the original
   and new public keys to the S3 bucket.

   Assign <Var name="proxy.example.com" /> to the domain of your Teleport
   Proxy Service, <Var name="my-aws-oidc" /> to the name of the AWS OIDC
   integration you fetched earlier, and <Var name="aws-role" /> to the name of
   the AWS IAM role created for the integration (the value of the
   `spec.aws_oidc.role_arn` field in the `integration` resource):

   ```code
   $ bash -c "$(curl 'https://<Var name="proxy.example.com" />/webapi/scripts/integrations/configure/awsoidc-idp.sh?integrationName=<Var name="my-aws-oidc" />&role=<Var name="aws-role" />')"
   ```

1. After rotation, we recommend re-running the script to remove the original
   public key and upload only the new one to S3.
...
1. If you lose connectivity to your resources, see if you need to reconfigure
   ...
   to reconfigure a database, then [roll back](#rollback) to the original
   certificate authority.
...
1. If you have lost connectivity to Teleport-protected resources, [roll
   back](#rollback) to the original certificate authority before entering the
...
1. Follow the instructions for your CA to ensure that you can connect to
   ...
   connectivity to Teleport-protected resources, [roll back](#rollback) to the
...
How [Teleport certificate authorities](../../architecture/authentication.mdx)
work.
